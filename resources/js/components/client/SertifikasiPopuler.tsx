  ...
  return (
    <section className="px-20 py-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
        {displaySertifikasi.slice(0, 4).map((item, idx) => (
          <div key={item.id || idx} className="border-2 border-purple-400 rounded-2xl overflow-hidden shadow bg-white flex flex-col min-w-[270px] md:min-w-[320px] max-w-full">
            ...
            <div className="p-6 flex-1 flex flex-col">
              ...
              <div className="flex gap-2 mt-auto">
                <a href={item.slug !== '#' ? `/detailsertifikasi/${item.slug}` : '#'} className="flex-1 px-4 py-2 rounded-lg bg-purple-700 text-white font-semibold text-center">Ambil Kelas</a>
                <a href={item.slug !== '#' ? `/detailsertifikasi/${item.slug}` : '#'} className="flex-1 px-4 py-2 rounded-lg border border-orange-400 text-orange-700 font-semibold text-center">Pelajari Kelas</a>
              </div>
            </div>
          </div>
        ))}
