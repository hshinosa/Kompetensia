                            ...
                            <DropdownMenuTrigger asChild>
                                <Button variant="ghost" className="size-10 rounded-full p-1">
                                    <Avatar className="size-8 overflow-hidden rounded-full">
                                        <AvatarImage 
                                            src={(auth.admin || auth.user)?.avatar as string | undefined} 
                                            alt={(auth.admin || auth.user)?.name as string | undefined} 
                                        />
                                        <AvatarFallback className="rounded-lg bg-neutral-200 text-black">
                                            {getInitials(((auth.admin || auth.user)?.name as string) || 'Admin')}
                                        </AvatarFallback>
                                    </Avatar>
                                </Button>
                            </DropdownMenuTrigger>
                            <DropdownMenuContent className="w-56" align="end">
                                <UserMenuContent user={(auth.admin || auth.user) as User} />
                            </DropdownMenuContent>
                        </DropdownMenu>
                    </div>
                </div>
